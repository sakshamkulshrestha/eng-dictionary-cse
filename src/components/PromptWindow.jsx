        ...
        <div className="apple-card group p-6 min-h-[170px] pb-16 flex flex-col focus-within:shadow-[0_24px_48px_rgb(0,0,0,0.06)] dark:focus-within:shadow-[0_24px_48px_rgb(0,0,0,0.4)]">
          ...
          <button 
            ...
            className={`absolute bottom-5 right-5 w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300 ${
              prompt.trim() ? 'bg-accent text-white shadow-lg shadow-accent/40 hover:shadow-accent/60 active:scale-90 hover:-translate-y-1' : 'bg-[#E5E5EA] dark:bg-[#2C2C2E] text-gray-400 dark:text-gray-500 cursor-not-allowed'
            }`}
          >
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
             {results.map((item, idx) => {
               ...
               <div 
                 ...
                 className={`apple-card cursor-pointer group flex flex-col min-h-[190px] overflow-hidden animate-slide-up hover:bg-accent-10/30 hover:border-accent/40 transition-all duration-300 ${delayClass}`}
               >
                 <div className="absolute top-0 right-0 p-5 opacity-0 group-hover:opacity-100 transition-all duration-500 -translate-x-2 group-hover:translate-x-0 z-10">
                 ...
                 <div className="relative z-10">
                   <div className="flex items-center gap-3 mb-5">
                     <span className="w-8 h-8 rounded-full bg-black/5 dark:bg-white/10 text-[13px] font-bold text-black dark:text-white flex items-center justify-center flex-shrink-0 group-hover:bg-accent group-hover:text-white transition-colors duration-300">
                       {idx + 1}
                     </span>
                     <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-black/5 dark:bg-white/10 text-[11px] font-bold text-gray-500 dark:text-gray-400 uppercase tracking-widest transition-colors duration-300 group-hover:bg-accent group-hover:text-white group-hover:shadow-md group-hover:shadow-accent/20">
                       <Hash className="w-3.5 h-3.5 flex-shrink-0" />
                       <span className="truncate">{getFullDomainName(item.domain)}</span>
                     </span>
                   </div>
                   
                   <h3 className="text-[22px] font-bold tracking-tight text-black dark:text-white mb-2 leading-tight pr-6 group-hover:text-accent transition-colors duration-300">
                     {item.term}
                   </h3>
                 </div>

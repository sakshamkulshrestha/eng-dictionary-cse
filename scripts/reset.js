...
// Transform ID from "1" to "DSA-001", "2" to "DSA-002", etc.
...
function transformTerm(term) {
  ...
  // 1. Change ID (e.g., "1" → "DSA-001")
  transformed.id = transformId(term.id);

  // 2. Transform term name (replace dashes with spaces)
...
async function main() {
  const inputPath = path.join(__dirname, 'data', 'dsa_terms.json');
  const outputJsonPath = path.join(__dirname, 'data', 'dsa_terms_transformed.json');
